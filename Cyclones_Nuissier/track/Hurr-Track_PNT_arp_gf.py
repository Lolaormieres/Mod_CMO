...
date_gf=datetime(int(annee),int(mois),int(jour),int(heure))

# ARP Atmo
# grid.arpege-forecast.glob01+0090:00.grib # Fichier atm qu'on cherche à lire
fill_atm=''
geometry_atm=domaine
cblock_atm='forecast'
formats_atm='grib'
ckind_atm='gridpoint'
cblock_atm='forecast'
...
if(domaine=='GLOB01'):
  ...
  print('**************************************************************** laaaaaaaaaaaaaaaaaaaaaaa')
  print('iie',iie)
  print('ije',ije)
...
#
    ...
    for jc in range(ech):
        ...
        zrmwmaxSE=0.
        #lecture  
        f=get_file_atm(expe,date_gf,cblock_atm,ckind_atm,cfields_atm,cmodel_atm,cut_atm,list_ech[jc],geometry_atm,formats_atm,fill_atm)
#        f = epygram.formats.resource('grid.'+model+'-forecast.'+domaine+'+'+str(list_ech[jc]).zfill(4)+':00.grib', 'r', 'GRIB')
        if(jc-1>=0):
          fprec=get_file_atm(expe,date_gf,cblock_atm,ckind_atm,cfields_atm,cmodel_atm,cut_atm,list_ech[jc-1],geometry_atm,formats_atm,fill_atm)
...
output1 = output(   
                output_formats = ['png'],
#                output_name = expe+"_track-radiuswind-"+bassin+'-'+tcname+"-"+str(ztime_basis[0]),
                output_name = expe+"_track-radiuswind-"+bassin+'-'+tcname+"-"+str(annee)+str(mois)+str(jour)+'_echmax_'+str(echmax),
                output_width = 1300, #1400                                             
