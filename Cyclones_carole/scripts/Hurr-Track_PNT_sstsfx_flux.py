...
def get_file(xpid,date,term,kind,geometry,fmt,model,origin,nativefmt,filling):
    #MODEL
#     geometry=domaine
     fp = {'experiment':xpid,
          'kind':kind,
...
for jc in range(ech):
  ...
  f=get_file(expe,date,int(list_ech[jc]),'gridpoint',domaine,'grib','arpege','historic','grib','')
  fsfx=get_file(expe,date,int(list_ech[jc]),'historic','global1798','fa','surfex','','fa','surf')
...
# lecture des variables
  ...
  sstr=fsfx.readfield('SFX.SST')
  flatr=f.readfield({'shortName':'slhf'})
  fsenr=f.readfield({'shortName':'sshf'})
  if (jc>0):
      fb=get_file(expe,date,int(list_ech[jc-1]),'gridpoint',domaine,'grib','arpege','historic','grib','')
...
#        
        ...
        sst=sstr.extract_point(lon,lat)
        sstval=float(sst.data)
        if (float(sst.data)>500.): sstval=-999.
