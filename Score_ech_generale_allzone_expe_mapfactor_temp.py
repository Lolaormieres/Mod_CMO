...
for izone, zone in enumerate(zone_car):
    ...
    df_ech = pd.read_csv(dir_carole+str(expe[0])+'_'+str(zone)+'_ech_septembre_octobre_prev-mer'+'.csv')
    df_ech.head()
    df_ech.dtypes
    ...
    for niv in name:
        ...
        ax.set_xticks(lst,
                labels=xlabels)
        axs[0].set_ylabel('Depth (m)',fontsize=26)
    
        
    ## PLOT PROPRE
        bar_tick=np.linspace(10, 28, 19)# GN84
        cbar_ax = fig.add_axes([0.4, -0.17, 0.2, 0.08])  # [left,bottom,width,height]
        CS1=ax.contourf(X, Y, Z,cmap='rainbow', levels=bar_tick,linewidths=1)
        clb=fig.colorbar(CS1, spacing='uniform', cax=cbar_ax, orientation='horizontal')
        cbar_ax.tick_params(labelsize=22)
        ax.tick_params(axis='y', labelsize=22)
        ax.tick_params(axis='x', labelsize=22)
        clb.ax.set_title('Temperature (°C)',fontsize=26)
        ...
        fig.suptitle('Evolution of the average temperature over the 102h forecast in 1d model - '+str(expe), horizontalalignment = 'center',fontsize=36, y=1.02)
    print('min',min(temp))
    print('max',max(temp))
    print('averaged',np.mean(temp))

figname = '/cnrm/recyf/Data/users/ormieresl/plot_score_water_column_temp/'+str(expe[0])+'_xptemp_column_ech_allzone.png'
fig.savefig(figname,dpi=150, format='png',bbox_inches='tight')
figname = '/cnrm/recyf/Data/users/ormieresl/plot_score_water_column_temp/'+str(expe[0])+'_xptemp_column_ech_allzone.pdf'
...
for izone, zone in enumerate(zone_car):
    ...
    ## PLOT PROPRE
        ...
        fig.colorbar(CS1, cax=cbar_ax, orientation='horizontal')
        # fig.suptitle('Evolution du biais de température par rapport à la Reference Mercator -'+str(expe), horizontalalignment = 'center',fontsize=15)
        fig.suptitle('Evolution of the averaged temperature of  Instananeous Mercator - '+str(expe), horizontalalignment = 'center',fontsize=36, y=1.02)
    print('min',min(temp))
    print('max',max(temp))
    print('averaged',np.mean(temp))

figname = '/cnrm/recyf/Data/users/ormieresl/plot_score_water_column_temp/'+str(expe[0])+'_xpmercator_column_ech_allzone.png'
fig.savefig(figname,dpi=150, format='png',bbox_inches='tight')
figname = '/cnrm/recyf/Data/users/ormieresl/plot_score_water_column_temp/'+str(expe[0])+'_xpmercator_column_ech_allzone.pdf'
fig.savefig(figname,dpi=150, format='pdf',bbox_inches='tight')
...
#         ax.tick_params(axis='y', labelsize=22)
#         ax.tick_params(axis='x', labelsize=22)
#         clb.ax.set_title('Bias (°C)',fontsize=26)
...
# figname = '/cnrm/recyf/Data/users/ormieresl/plot_score_water_column/Scores_echeance_water_columns_'+str(zone[0])+'_'+str(expe[0])+'_'+str(expe[1])+'_biais.png'
# fig.savefig(figname,dpi=150, format='png',bbox_inches='tight')


# figname = '/cnrm/recyf/Data/users/ormieresl/plot_score_water_column/Scores_echeance_water_columns_'+str(zone[0])+'_'+str(expe[0])+'_'+str(expe[1])+'_biais.pdf'
# fig.savefig(figname,dpi=150, format='pdf',bbox_inches='tight')