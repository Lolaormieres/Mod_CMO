...
begdat = tools.date.Date(2022,7,29,3)
enddat = tools.date.Date(2022,11,6,15)#31,21)#6,30,18)
dateList=[]
d=begdat

while (d <= enddat):
    dateList.append(d)
    d = d + tools.date.Period(hours=3)
...
baserequest_can = "odbsql -q 'select lon, lat, date, time, statid, varno, obsvalue, fg_depar, an_depar, datum_anflag.final, from hdr,body, where varno=39 or varno=58 or varno=11 or varno=92' "
...
for dat in dateList:
    ...
    for xp in [xpid_dble,xpid_ref]:
        ...
        os.system(baserequest_can+" -i ECMA_CAN -funetcdf -o "+file_netcdf_can)
        os.system('rm -rf ECMA*')
        os.chdir(oldpwd)
