...
for i in range(len(name)):
    ...
    mask=(lon>long)&(lon<long+1)&(lat>lati-1)&(lat<lati) #masques sur lon et lat OK
    ...
    f_for_moins_pt=[f_for_moins_total[0,0]]
...
plt.figure()
plt.title('Evaluation of temperature vertical profile'+' '+str(date)[0:10]+'-'+str(date)[11:13]+', lon:{},lat:{}'.format(long,lati), fontsize=17, y=1.05)
...
plt.plot(x_for,level,linewidth=1,label=str(expe)+'_P96_prod - 13/09',color=color_expe, linestyle='--') # P48
