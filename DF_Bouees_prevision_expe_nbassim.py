...
pathres='/home/ormieresl/Routines/DF_bouees_ech_vf/'
...
def get_file(xpid,date,rep,typfic,fields,mod,chain,ech,geo,formats,fill):
    ...
    fp = {'experiment':xpid,'kind':typfic,'block':rep,'fields':fields,
          ...
          'local':'tmpb_ech.fic','cutoff':chain,'vapp':'arpege',
          'vconf':'4dvarfr','model':mod,'namespace':'vortex.multi.fr'}
    fcref = toolbox.input(**fp)[0]
    fcref.get()
    r = epygram.formats.resource(filename='tmpb_ech.fic',openmode='r')
    os.remove('tmpb_ech.fic')
    return r



#%%
# #Test for lon/lat of zones --> mask
# #Read NetCDF
# chemin_nc = '/cnrm/recyf/Work/temp/ormieresl/NO_SAVE/cache/vortex/arpege/4dvarfr/'+str(expe[0])+'/'
        ...
        for t in range(nb):
            ...
            #cterm recuperer heure pour bouees
               ...
               if os.path.isfile(fic_nc):
                    ...
                    #obs value tot
                    data2plota_tot = odb_key_val_dic['col_7'][ind_flag_obs]     ## 7 obsvalue tot sans indflag_datum
                    ...
                    Nb_obs_tot=len(data2plota_tot)
                    Nb_diff=Nb_obs_tot-Nb_obs_assim  #Diff between nubers of obs tot and numbers of obs assim
...
for zon in zones.keys():
    ...
    df_bouees_final[zon].to_csv(pathres+str(expe[e])+'_'+str(zon)+'_prevision_bouees_flag_ech.csv')
