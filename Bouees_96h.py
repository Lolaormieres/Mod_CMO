...
pathres="/home/ormieresl/Scripts/bouees/DF_bouees_96h/"
...
zones = {"nordat": [-10,0,20,30], "eurat": [-35,0,20,30]} # "tropiques": [0,360,-20,20],"hn20":[0,360,20,90],"hs20":[0,360,-90,-20],"glob":[0,360,-90,90]}
...
#        data_zon[zon]=pd.DataFrame(raw_data, columns = ['date_xp','ech','date_mer','cterm_mer','expe','var','level','biais','eqm','zone','compar','temp_exp_mean','temp_merc_mean'])
    ...
    for e in range(len(expe)):
        ...
        for t in range(nb):
            ...
            timedelta = dt.timedelta(0,0,0,0,0,Delta)
            toto_xp=[Date_debut_xp+i*timedelta for i in range(nbDate)]
            reseaux=""
            ...
            for key in zones.keys():
                ...
                #Flag zone
                ind_flag = np.where((odb_key_val_dic['col_1']<zones[key][1])&
                (odb_key_val_dic['col_1']>zones[key][0])&
    ...
    #df_bouees_for.to_csv(pathres+"/"+str(zone)+'_P96-bouees.csv')
...
## Enregistre df separé
    for zon in zones.keys():
        print('zon',zon)
        print('df_bouees_for[zon]',df_bouees_for[zon])
        df_bouees_for[zon].to_csv(pathres+"/"+str(zon)+'_P96-bouees'+'.csv')
