...
for k,j in np.ndindex(n,m):
   tabdiv[k,j]=np.divide(tabval[k,j],(6*3600))
x_ref.setdata(tabdiv)

crs=ccrs.PlateCarree()

fig, ax = (x_expe-x_ref).cartoplot(projection=crs,plot_method='scatter',minmax=MINMAX,colormap='biais_lat_thiner')

fig, ax = (x_ref).cartoplot(projection=crs,plot_method='scatter',minmax=MINMAX,colormap='biais_flx_lat')
...
figname=dir_im+'Compar_'+str(expe)+'-'+str(xpref)+'_'+str(date)[0:10]+'-'+str(date)[11:13]+str(cterm_for)+'difftest_'+str(var)+'.watt_thiner.png'
figname=dir_im+'val_'+str(xpref)+'_'+str(date)[0:10]+'-'+str(date)[11:13]+str(cterm_for)+str(var)+'.watt_thiner.png'

fig.savefig(figname, dpi=100, format='png',bbox_inches=None, pad_inches=0.1)
