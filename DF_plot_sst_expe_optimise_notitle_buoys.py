...
for izone, zone in enumerate(zone):
    ...
    ax[0].plot(df_GN84.date,df_GN84.biais,label='Xp.final',color=color_GN84,linewidth=3,alpha=0)
    ...
    ax[1].plot(df_GN84.date,df_GN84.eqm,label='Xp.final',color=color_GN84,linewidth=3,alpha=0)
    ...
    # ax[1].legend(['L.26','L.12','R.50','L.12.noccurr','L.12.nocurr.drown','Xp.finale','Xp.buyos','Xp.wind','Xp.R10x4','oper.2024'],fontsize=19,bbox_to_anchor=(1.1, 1.05))
    # ax[1].legend(['L.26','L.12','R.50','Xp.finale','Xp.buyos','Xp.R10x4','Ref.noOML'],fontsize=19,bbox_to_anchor=(1.1, 1.05))
    # ax[1].legend(['L.26','L.12','R.50','min.bathy','Xp.final','Ref.noOML'],fontsize=24,bbox_to_anchor=(1.1, 1.05))

    ax[1].set_ylim(0, 0.7)
    
    biais_moy=np.round(df_GKCJ.biais.mean(),3)
    std_moy=np.round(df_GKCJ.eqm.mean(),3)
    new_row = {'zone':zone,'expe':'GN84','biais':biais_moy, 'eqm':std_moy}    
    ...
    df.to_csv(dir_fig+'analyse_sst_REF.csv')
...
for izone, zone in enumerate(zone):
    ...
    # ax[0].plot(df_GM6E.date,df_GM6E.biais,label='no.curr',color=color_GM6E,linewidth=3)
    ax[0].plot(df_GMOT.date,df_GMOT.biais,label='min.bathy',color=color_GMOT,linewidth=3)
    ax[0].plot(df_GN84.date,df_GN84.biais,label='Xp.OML',color=color_GN84,linewidth=3,alpha=0)
    # ax[0].plot(df_GO4A.date,df_GO4A.biais,label='GO4A',color=color_GO4A,linewidth=3)
    # ax[0].plot(df_GNSR.date,df_GNSR.biais,label='GNSR',color=color_GNSR,linewidth=3)
    # ax[0].plot(df_GNYG.date,df_GNYG.biais,label='GNYG',color=color_GNYG,linewidth=3)
    # ax[0].plot(df_GOJQ.date,df_GOJQ.biais,label='GOJQ',color=color_GOJQ,linewidth=3)
    ...
    # ax[1].legend(['L.26','L.12','R.50','L.12.noccurr','L.12.nocurr.drown','Xp.finale','Xp.buyos','Xp.wind','Xp.R10x4','oper.2024'],fontsize=19,bbox_to_anchor=(1.1, 1.05))
    # ax[1].legend(['L.26','L.12','R.50','Xp.finale','Xp.buyos','Xp.R10x4','REF.noOML'],fontsize=19,bbox_to_anchor=(1.1, 1.05))
    ax[1].legend(['L.26','L.12','R.50','min.bathy','Xp.final','REF.noOML'],fontsize=27,bbox_to_anchor=(1.05, 1.05))
    
    biais_moy=np.round(df_GKCJ.biais.mean(),3)
    std_moy=np.round(df_GKCJ.eqm.mean(),3)
    Nb_moy=np.round(df_GKCJ.Nb_data_diff.mean())
    
    new_row = {'zone':zone,'expe':'GN84','biais':biais_moy, 'eqm':std_moy,'Nb':Nb_moy}    
    df2.loc[len(df2)] = new_row
      
    # SUPRIMER 1er COL DF
    #df2.drop(index=df2.index[0], axis=0, inplace=True) ## DELETE FIR
    df2.to_csv(dir_fig+'forecast_sst_REF.csv')
    
    
    plt.show()
    figname = dir_fig+'SST_forecast-mercator'+'_'+str(zone)+'.png'
    fig2.savefig(figname,dpi=150, format='png',bbox_inches='tight')
    figname = dir_fig+'SST_forecast-mercator'+'_'+str(zone)+'.pdf'
