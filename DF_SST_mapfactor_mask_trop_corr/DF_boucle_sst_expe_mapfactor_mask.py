...
# directory
Dir = '/home/ormieresl/Routines/DF_SST_mapfactor_mask_trop_corr'
...
# expe = ['GKOR','GKPH', 'GMOT', 'GM6E','GNSR','GNYG']
# expe_mer = ['GKOR','GKPH','GMOT','GM6E','GNSR','GNYG']
...
def get_file(xpid, date, rep, typfic, fields, mod, chain, ech):
    ...
    fp = {'experiment': xpid, 'kind': typfic, 'block': rep, 'fields': fields,
          ...
          'local': 'tmpsst_expe.fic', 'cutoff': chain, 'vapp': 'arpege',
          'vconf': '4dvarfr', 'model': mod, 'namespace': 'vortex.multi.fr', 'filling': 'surf'}
    fcref = toolbox.input(**fp)[0]
    fcref.get()
    r = epygram.formats.resource(filename='tmpsst_expe.fic', openmode='r')
    os.remove('tmpsst_expe.fic')
...
zones = {"tropiques": [-180, 180,-20, 20]}

# # %%
# expe = ['GL55','GKOR','GKPH', 'GMOT', 'GM6E','GN84','GO4A','GNSR','GKCJ','GK7C','GOJQ']
# expe_mer = ['GL55','GKOR','GKPH', 'GMOT', 'GM6E','GN84','GO4A','GNSR','GN84','GL55','GOJQ']

expe = ['GL55','GKOR','GKPH','GN84','GMOT','GO4A','GKCJ','GOJQ']
expe_mer = ['GL55','GKOR','GKPH','GN84','GMOT','GO4A','GN84','GOJQ']
# =============================================================================
...
raw_data = {'date': [''],
                ...
                'ech': [''],
                'zone': [''],
                'compar': [''], 'SST_xp': [''], 'SST_mer': [''], 'Nb_data_diff':[''],'Nb_data_xp':[''],'Nb_data_mer':['']}

for e in range(len(expe)):
    df = {}
    for zon in zones.keys():
        df[zon] = pd.DataFrame(raw_data, columns=['date', 'expe', 'var', 'level',
                                'biais', 'eqm', 'ech', 'zone', 'compar', 'SST_xp', 'SST_mer','Nb_data_diff','Nb_data_xp','Nb_data_mer'])
    dt = timedelta(days=1)
    date=date_init 
    ...
    for t in range(nb):
            ...
            try:
                ...
                for i in range(len(name)):
                    ...
                    SST_xp_tot=r.readfield(name_sst[i]) - 273.15
                    SST_mer_tot=r_mer.readfield(name_mer[i]) - 273.15  # Surf theta Kelvin
                    f_diff=r.readfield(name_sst[i])-(r_mer.readfield(name_mer[i]))
                     
                    #get map factor of stretch grid
                    map_factor = (f_diff.geometry.map_factor_field()).getdata()
                    for key in zones.keys():
                        #print('keyyyy',key)
                      maskdm=(lon > zones[key][0]) & (lon < zones[key][1]) & (
                      ...
                      #temp
                      SST_xp   = sgmeandm(SST_xp_tot.data,map_factor,maskdm)
                      SST_mer  = sgmeandm(SST_mer_tot.data,map_factor,maskdm)
                      
                      #calcul nb de pt de grille par jour moyen - sur lequel est calcule le biais
                      Nb_data=len(f_diff.data.data[maskdm][np.invert(f_diff.data[maskdm].mask)])
                      Nb_data_xp=len(SST_xp_tot.data.data[maskdm][np.invert(SST_xp_tot.data[maskdm].mask)])                    
                      Nb_data_mer=len(SST_mer_tot.data.data[maskdm][np.invert(SST_mer_tot.data[maskdm].mask)]) 
                     
                      print('*******************************************************************Nb_data', Nb_data)
                      # avant correction  
                      # Nb_data=len(f_diff.data[mask])
                      # Nb_data_xp=len(SST_xp_tot.data[mask])
                      # Nb_data_mer=len(SST_mer_tot.data[mask])
                      ...
                      new_row={'date': date_2, 'expe': expe[e], 'var': name_sst[i], 'level': level[i], 'biais': f_diff_mean,
                              'eqm': f_diff_std,'ech':cterm, 'zone': key, 'compar': compar, 'SST_xp': SST_xp, 'SST_mer': SST_mer, 'Nb_data_diff': Nb_data,'Nb_data_xp': Nb_data_xp,'Nb_data_mer':Nb_data_mer}
...
#     20, 20], "hn20": [0, 360, 20, 90], "hs20": [0, 360, -90, -20], "glob": [0, 360, -90, 90], "med": [-3,16,30,44]}
compar = 'prev-mer_sst'
ech = cterm
dt = timedelta(days=1)
# # =============================================================================
...
for e in range(len(expe)):
    ...
    for t in range(nb):
            ...
            try:
                ...
                r_mer = get_file(expe_mer[e], date_valid, cblock_oc, ckind_oc, cfields_oc, cmodel_oc, cut_oc, cterm)
                X = r_mer.geometry.get_lonlat_grid()  # lon et lat
                lon = X[0]
                lat = X[1]
                ...
                for i in range(len(name)):
                    print(i, name[i])
                    # SST arp Kelvin, TempOC °C
                    SST_xp_tot=r_for.readfield(name_sst[i]) - 273.15
                    SST_mer_tot=r_mer.readfield(name_mer[i]) - 273.15  # Surf theta Kelvin
    
                    f_diff=r_for.readfield(name_sst[i])-(r_mer.readfield(name_mer[i]))
                    #get map factor of stretch grid
                    map_factor = (f_diff.geometry.map_factor_field()).getdata()
                    for key in zones.keys():
                        #print('keyyyy',key)
                      ...
                      # Define the new row to be added
                      new_row={'date': date_valid_2, 'expe': expe[e], 'var': name_sst[i], 'level': level[i], 'biais': f_diff_mean,
                              'eqm': f_diff_std, 'ech':cterm_for,'zone': key, 'compar': compar, 'SST_xp': SST_xp, 'SST_mer': SST_mer, 'Nb_data_diff': Nb_data,'Nb_data_xp':Nb_data_xp,'Nb_data_mer':Nb_data_mer}
                      # Use the loc method to add the new row to the DataFrame
...
for e in range(len(expe)):
    df3 = {}
    for zon in zones.keys():
        df3[zon] = pd.DataFrame(raw_data, columns=['date', 'expe', 'var', 'level',
                                'biais', 'eqm', 'ech', 'zone', 'compar', 'SST_xp', 'SST_mer','Nb_data_diff','Nb_data_xp','Nb_data_mer'])
    dt = timedelta(days=1)
    dt2 = timedelta(days=4)
    date=date_init 
    ...
    for t in range(nb):
            ...
            try:
                #r = get_file(expe[e], date, cblock, ckind,cfields, cmodel, cut, cterm)
                r_for=get_file(expe[e],date,cblock_for,ckind_for,cfields,cmodel,cut_for,cterm_double)
                r_mer = get_file(expe_mer[e], date_valid, cblock_oc, ckind_oc, cfields_oc, cmodel_oc, cut_oc, cterm)
                X = r_mer.geometry.get_lonlat_grid()  # lon et lat
                lon = X[0]
                lat = X[1]
                ...
                for i in range(len(name)):
                    ...
                    for key in zones.keys():
                      ...
                      # Avant correction
                      # Nb_data=len(f_diff.data[maskdm])
                      # Nb_data_xp=len(SST_xp_tot.data[maskdm])
                      # Nb_data_mer=len(SST_mer_tot.data[maskdm])
                      ...
                      new_row={'date': date_valid_2, 'expe': expe[e], 'var': name_sst[i], 'level': level[i], 'biais': f_diff_mean,
                              'eqm': f_diff_std, 'ech':cterm_for,'zone': key, 'compar': compar, 'SST_xp': SST_xp, 'SST_mer': SST_mer, 'Nb_data_diff':Nb_data,'Nb_data_xp':Nb_data_xp,'Nb_data_mer':Nb_data_mer}
                      # Use the loc method to add the new row to the DataFrame
                      df3[key].loc[len(df3[key])]=new_row
...
## Enregistre df separé
    for zon in zones.keys():
        print('zon',zon)
        print('df[zon]',df3[zon])
        df3[zon].to_csv(Dir+"/"+str(expe[e])+'_'+str(zon)+'_'+str(compar)+'.csv')
