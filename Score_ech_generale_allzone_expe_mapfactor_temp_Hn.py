...
raw_data = {'zone': [''],
                ...
                'biais': [''],
                'eqm': ['']}
df2 = pd.DataFrame(raw_data, columns = ['zone','expe','biais','eqm']) ## dataframe stat moyennes
...
for izone in range(len(zone)):
    ...
    for niv in name:
        ...
        ax.set_title('Xp.OML',fontsize=26,y=1.06)
    ...
    ## PLOT PROPRE
        ...
        cbar_ax = fig.add_axes([1, 0.22, 0.02, 0.6]) # vertical
        ...
        # df_ech stat moyennes
        biais_moy=np.round(df_ech.biais.mean(),1)
        std_moy=np.round(df_ech.eqm.mean(),1)
        # Nb_moy=np.round(df_ech.Nb.mean())
    
        new_row = {'zone':zone[izone],'expe':expe,'biais':biais_moy, 'eqm':std_moy}    
        df2.loc[len(df2)] = new_row
          
        # SUPRIMER 1er COL DF
        #df2.drop(index=df2.index[0], axis=0, inplace=True) ## DELETE FIR
        df2.to_csv(dir_fig3+'bouees_september.csv')
...
for izone in range(len(zone)):
    ...
    for niv in name:
        ...
        ax.set_title('Mercator.instantaneous', fontsize=26,y=1.06)
    ...
    ## PLOT PROPRE
        cbar_ax = fig.add_axes([1, 0.22, 0.02, 0.6]) 
        CS1=ax.contourf(X, Y, Z,cmap='rainbow', levels=bar_tick[izone],linewidths=1)
        clb=fig.colorbar(CS1, spacing='uniform', cax=cbar_ax, orientation='vertical')
        cbar_ax.tick_params(labelsize=20)
        ax.tick_params(axis='y', labelsize=22)
