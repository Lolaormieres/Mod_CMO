...
geometry='global1798'
#term='24'
dir_im='/cnrm/recyf/Data/users/ormieresl/plot_cartes_diff/'
...
def plot_diffmean():
    for i in range(len(name)):
        print(i,name[i])
        crs=ccrs.PlateCarree()
        fig, ax = x_diff.cartoplot(projection=crs,plot_method='scatter',colormap='biais_v4_grey') # biais_v4_grey analyse 30j, biais_v4_grey_v3 forecast
        ax.set_extent([-180, 180, -90, 90], ccrs.PlateCarree())
        ax.title.set_text(name[i]+'_'+str(expe[e])+'-'+str(xmer)+'_'+str(date_init)[0:10]+'-'+str(date_init)[11:13]+'_'+str(nb)+'jours')
        figname=dir_im+'_'+name[i]+'_'+str(expe[e])+'-'+str(xmer)+'_'+str(date_init)[0:10]+'-'+str(date_init)[11:13]+'_'+str(nb)+'jours.png'
        # ax.title.set_text(name[i]+'_'+str(expe)+'-'+str(xmer)+'_'+str(date_init)[0:10]+'-'+str(date_init)[11:13]+'_'+str(cterm_for)+'_'+str(nb)+'jours'+'-GLOB')
        # figname=dir_im+'_'+name[i]+'_'+str(expe)+'-'+str(xmer)+'_'+str(date_init)[0:10]+'-'+str(date_init)[11:13]+'_'+str(cterm_for)+'_'+str(nb)+'jours'+'-GLOB.png'
        fig.savefig(figname, dpi=300, format='png',bbox_inches=None, pad_inches=0.1)
...
#Diff journaliere
nb=30
date_init=datetime(2022,10,2,0)
date=date_init
dt=timedelta(days=1)
dt_for=timedelta(days=4)

date_valid=date_init+dt_for
cterm_for='96'
...
expe=['GL55', 'GKOR', 'GKPH','GN84', 'GO4A', 'GOJQ']
# for e in range(len(expe)): 
for e in range(len(expe)):
    ...
    for t in range(nb):
        ...
        try:
            ...
            r_mer = get_file(expe[e], date, cblock_oc, ckind_oc,cfields_oc, cmodel_oc, cut_oc, cterm)
            if t == 0:
                x_diff = r.readfield(name[0])-(r_mer.readfield(name_mer[0])-273.15)
            else:
                x_diff=r.readfield(name[0])-(r_mer.readfield(name_mer[0])-273.15)+x_diff
        except:
            print('Pas de fichier')
        date=date+dt
