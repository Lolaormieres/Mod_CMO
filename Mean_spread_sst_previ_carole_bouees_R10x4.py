...
for izone in range(len(zone)):
    ...
    sns.lineplot(data=valnonanew_bouees,x="ech",y="biais_obs",label=liste_param[2],color=liste_color[2],linewidth=2)
    sns.lineplot(data=valnonanew_bouees,x="ech",y="biais_obs",label=liste_param[3],color=liste_color[3],linewidth=2)
    ...
    axes.grid(alpha=0.8)
    # plt.title(name[jlev])
    plt.savefig(dir_im+zone[izone]+"Plot_IC_REF_OML_eval_bouees_R10x4.png")
plt.cla()
