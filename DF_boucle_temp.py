...
def get_file(xpid, date, rep, typfic, fields, mod, chain, ech):
    ...
    fp = {'experiment': xpid, 'kind': typfic, 'block': rep, 'fields': fields,
          ...
          'local': 'tmptemp2.fic', 'cutoff': chain, 'vapp': 'arpege',
          'vconf': '4dvarfr', 'model': mod, 'namespace': 'vortex.multi.fr', 'filling': 'surf'}
    fcref = toolbox.input(**fp)[0]
    fcref.get()
    r = epygram.formats.resource(filename='tmptemp2.fic', openmode='r')
    os.remove('tmptemp2.fic')
    ...
    for t in range(nb):
            ...
            for i in range(len(name)):
               ...
               try:
                   ...
                   SST_xp_tot=r.readfield(name[i]) # SST arp Kelvin, TempOC °C
                   ...
                   for key in zones.keys():
                       ...
                       new_row = {'date':date_2, 'expe':expe[e], 'var':name[i],'level':level[i], 'biais':f_diff_mean,'eqm':f_diff_std, 'ech':cterm,'zone':key,'compar':compar, 'SST_xp':SST_xp,'SST_mer':SST_mer}
    ...
    for t in range(nb):
            ...
            for i in range(len(name)):
               ...
               try:
                   ...
                   SST_xp_tot=r_for.readfield(name[i]) # SST arp Kelvin, TempOC °C
                   SST_mer_tot=r_mer.readfield(name_mer[i]) - 273.15  # Surf theta Kelvin
                   ...
                   for key in zones.keys():
                       ...
                       # Define the new row to be added
                       new_row = {'date':date_2,'date_valid':date_valid_2, 'expe':expe[e], 'var':name[i],'level':level[i], 'biais':f_diff_mean,'eqm':f_diff_std, 'ech':cterm,'zone':key,'compar':compar, 'SST_xp':SST_xp,'SST_mer':SST_mer}
                       # Use the loc method to add the new row to the DataFrame
