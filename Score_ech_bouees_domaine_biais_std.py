...
Dir_im='/cnrm/recyf/Data/users/ormieresl/plot_ech_bouees_v2/'
...
for izone, zon in enumerate(zone):
    ...
    fig, axs = plt.subplots(nrows = 1,
                                    ncols = 1, 
                                    sharey = True, 
                                    figsize=(12, 5),dpi=150) #instead 25,5
    for isim, sim in enumerate(expe):
        ...
        ax.plot(df_res.ech,df_res.biais_obs,color=color[isim],linewidth=2, label=name_exp[isim])
        ...
        axs.set_ylabel('°C',fontsize=16)
        ax.set_xlabel('Forecast range (hours)',fontsize=16)
        ax.set_title(str(name_zone[izone]), fontsize = 18, y= 1.05)
        ...
        ax2 = axs.twinx()
        ax2.plot(df_res.ech,df_buoys.Nb_obs.astype(int),color=color[isim], linestyle=':',linewidth=1.4)
        stepsize2=200
        start2=0
        end2=2200
        stepsize2=12
        ...
        ax3 = axs.twinx()
        ax3.yaxis.set_ticks([])
        ax3.set_ylabel('Buoys number',fontsize=15)
        ...
        ax.legend(fontsize=14, bbox_to_anchor=(1.1, 0.2)) 
     
        # fig.savefig(Dir_im+'biais_std_bouees_allzones_mask_HN_GOJQ.png',bbox_inches="tight")
    fig.savefig(Dir_im+str(zone[izone])+'_biais_std_bouees_allzones_mask_HN_REF_R10x4.png',bbox_inches="tight",dpi=160)
