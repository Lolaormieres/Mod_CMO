...
def get_file(xpid,date,rep,typfic,fields,mod,chain,ech):
    ...
    fp = {'experiment':xpid,'kind':typfic,'block':rep,'fields':fields,
          ...
          'local':'tmpblanche.fic','cutoff':chain,'vapp':'arpege',
          'vconf':'4dvarfr','model':mod,'namespace':'vortex.multi.fr','filling':'surf'}
    fcref = toolbox.input(**fp)[0]
    fcref.get()
    r = epygram.formats.resource(filename='tmpblanche.fic',openmode='r')
    os.remove('tmpblanche.fic')
    return r
...
def plot_diffmean():
    for i in range(len(name)):
        ...
        fig, ax = x_diff.cartoplot(projection=crs,minmax=MINMAX,plot_method='scatter',colormap='biais_v4')
        # ax.set_extent([-180, 180, -90, 90], ccrs.PlateCarree())
        ax.set_extent([-35, 45, 0,55], ccrs.PlateCarree())
        ...
        # ax.set_title('Diurnal SST range in OML model (xp.final), average over september (18UTC - 6UTC)', fontsize=26,y=1.02)
        ...
        # figname = '/cnrm/recyf/Data/users/ormieresl/mean_diurnal_amplitude_GN84'+'_'+str(zone)+'.png'
        # fig.savefig(figname,dpi=300, format='png',bbox_inches='tight')
        # figname = '/cnrm/recyf/Data/users/ormieresl/mean_diurnal_amplitude_GN84'+'_'+str(zone)+'.pdf'
...
#try:
    ...
    if t == 0:
        x_diff = r_moins.readfield(name[0])-(r_plus.readfield(name[0])) *0
    else:
        x_diff=r_moins.readfield(name[0])-(r_plus.readfield(name[0]))+x_diff *0
    # except:
    #     print('Pas de fichier')
    date=date+dt
    date_plus=date+dh
...
#     mask=(lon_mer<45)&(lon_mer>-35)&(lat_mer>20)&(lat_mer<72)&(f.data > 200) & (f.data < 400)
    
#     if t == 0:
#         x_diff = r_mer_plus.readfield(name_mer[0])-(r_mer_moins.readfield(name_mer[0]))
      
#     else:
#         x_diff=r_mer_plus.readfield(name_mer[0])-(r_mer_moins.readfield(name_mer[0]))+x_diff
...
# print('mean diurnal amplitudes - MERC: ',np.round(x_diff.data[mask].mean(),2))
