...
for e in range(len(expe)):
    ...
    for t in range(nb):
        ...
        lon_expe=X[0]
        lat_expe=X[1]
        x=r.readfield(name_sst[0]) - 273.15
...
df_bouees_for.to_csv(pathres+str(zone)+'_'+'_A'+str(hour)+'h-bouees_double_flag.csv')  
...
# hour=6
# df_bouees_double= pd.read_csv('DF_bouees_vf/'+str(zone)+'_'+str(cterm)+'_analyse_'+str(hour)+'h-bouees_double.csv')
...
# df_bouees= pd.read_csv('DF_bouees_vf/'+str(zone)+'_'+str(cterm)+'_analyse_'+str(hour)+'h-bouees.csv')
# df_bouees.head()
# df_bouees.dtypes
