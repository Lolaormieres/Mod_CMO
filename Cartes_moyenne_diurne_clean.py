...
Dir_im='/cnrm/recyf/Data/users/ormieresl/plot_cartes_diurne_v4/'
...
def plot_diffmean():
    for i in range(len(name)):
        ...
        # ax.set_extent([-3, 16, 30, 44], ccrs.PlateCarree())
        # ax.set_extent([-80,0,20,70], ccrs.PlateCarree())
        # ax.set_extent([-180,180,20,90], ccrs.PlateCarree())
        # ax.set_extent([-180,180,-20,20], ccrs.PlateCarree())
        # ax.set_extent([-180,180,-90,-20], ccrs.PlateCarree())
        # ax.set_extent([110,-20,20,72], ccrs.PlateCarree())
       
        # # ax.title.set_text(name[i]+'_'+str(expe)+'-'+str(xmer)+'_'+str(date_init)[0:10]+'-'+str(date_init)[11:13]+'_'+str(nb)+'jours'+'-GLOB-')
        ax.title.set_text(name[i]+'_'+str(expe)+'_'+str(date_init)[0:10]+'-'+str(date_init)[11:13]+'_'+str(nb)+'jours '+str(cterm_18)+'-'+str(cterm_6))
        ...
        figname = Dir_im+str(expe)+'_'+str(zone)+'_diurnal_amplitude_'+str(nb)+'jours_'+str(cycle)+'february.png'
        fig.savefig(figname,dpi=150, format='png',bbox_inches='tight')
        figname = Dir_im+str(expe)+'_'+str(zone)+'_diurnal_amplitudes_'+str(nb)+'jours_'+str(cycle)+'february.pdf'
        ...
        # # gl.xformatter = LONGITUDE_FORMATTER
        # # gl.yformatter = LATITUDE_FORMATTER
        # gl.xlabel_style = {'size': 19, 'color': 'black', 'weight':'bold'}
        # gl.ylabel_style = {'size': 19, 'color': 'black','weight':'bold'}
        ...
        # figname = Dir_im+str(expe)+'_'+str(zone)+'_diurnal_amplitude_mercator_'+str(nb)+'jours.pdf'
...
#Diff journaliere
nb=30
date_init=datetime(2022,9,2,0) # put 0 for CMO and 6 for Mercator
date=date_init
...
#try:
    r_moins = get_file(expe, date_plus, cblock, ckind, cfields, cmodel, cut, cterm)
    r_plus = get_file(expe, date, cblock, ckind, cfields, cmodel, cut, cterm)
    ...
    mask=(lon<45)&(lon>-35)&(lat>20)&(lat<72) # Eurat
    # mask=(lon<16)&(lon>-3)&(lat>30)&(lat<44) # Med
    # mask=(lon<0)&(lon>-80)&(lat>20)&(lat<70) #Natl
    # mask=(lon<180)&(lon>-180)&(lat>-20)&(lat<20) #Tropivs
    # mask=(lon<180)&(lon>-180)&(lat>-90)&(lat<-20) #Hs
    
    # mask=(lon>110)&(lon<-20)&(lat>20)&(lat<72) #Npac
...
print('mean diurnal amplitudes total - CMO: ',np.round(x_diff.data.mean(),3))
print('min diurnal amplitudes total - CMO: ',np.round(x_diff.data.min(),3))
print('max diurnal amplitudes total - CMO: ',np.round(x_diff.data.max(),3))

print('mean diurnal amplitudes - CMO: ',np.round(x_diff.data[mask].mean(),3))
print('min diurnal amplitudes - CMO: ',np.round(x_diff.data[mask].min(),3))
print('max diurnal amplitudes - CMO: ',np.round(x_diff.data[mask].max(),3))
plot_diffmean()
...
#     # mask=(lon_mer<16)&(lon_mer>-3)&(lat_mer>30)&(lat_mer<44) & (f.data > 200) & (f.data < 400) # Med
#     # mask=(lon_mer<0)&(lon_mer>-80)&(lat_mer>20)&(lat_mer<70) & (f.data > 200) & (f.data < 400) # Natl
#     # mask=(lon_mer<180)&(lon_mer>-180)&(lat_mer>20)&(lat_mer<90) & (f.data > 200) & (f.data < 400) # Hn
#     # mask=(lon_mer<180)&(lon_mer>-180)&(lat_mer>-20)&(lat_mer<20) & (f.data > 200) & (f.data < 400) # Tropics
#     # mask=(lon_mer<180)&(lon_mer>-180)&(lat_mer>-90)&(lat_mer<-20) & (f.data > 200) & (f.data < 400) # Hs
