...
color_GP84='lightsteelblue'

color_GO50='darkgreen'
#import bronx
epygram.init_env()

dir_im = '/cnrm/recyf/Data/users/ormieresl/plot_param_cyclones_v1/'
dir_df = '/home/ormieresl/Routines/Cyclones_caroles_csv/'

dir_df_direct ='/home/ormieresl/Cyclones_carole/Exp/'

dir_obs= '/cnrm/recyf/Data/users/ormieresl/cyclones_obs/'
...
cyclone = str(nom_cyclone)+'_'+str(date)+'_00.csv'
...
df_GPCJ = pd.read_csv(dir_df_direct+'GPCJ/glob01/'+str(cyclone), names=colnames, header=None)

df_GP7E = pd.read_csv(dir_df_direct+'GP7E/glob01/'+str(cyclone), names=colnames, header=None)

df_GP84 = pd.read_csv(dir_df_direct+'GP84/glob01/'+str(cyclone), names=colnames, header=None)

df_GPGP = pd.read_csv(dir_df_direct+'GPGP/glob01/'+str(cyclone), names=colnames, header=None)

df_obs = pd.read_csv(dir_obs+nom_cyclone+'.csv', names=colnames_obs, header=0)


# df = pd.read_csv('/home/ormieresl/Cyclones_carole/IANobsibt.csv', header=True)

# df2=df.loc[df[6]>'2022-09-27 00:00:00']
# df3=df2.loc[df[6]<'2022-09-30 00:00:00']

# df3.to_csv('/cnrm/recyf/Data/users/ormieresl/cyclones_obs/test1.csv')
#-----------------------------------------------------------------------------
# Pmin
...
ax2.plot(df_GP7E.iloc[:,2],df_GP7E.iloc[:,13],linewidth=1.5,zorder=1, label = "Xp.OML.buoys",color=color_GO4A) 
ax2.plot(df_GP84.iloc[:,2],df_GP84.iloc[:,13],linewidth=1.5,zorder=1, label = "XP.OML.buoys.off",color=color_GP84) 
ax2.plot(df_GPGP.iloc[:,2],df_GPGP.iloc[:,13],linewidth=1.5,zorder=1, label = "XP.OML.buoys.R10x4",color=color_GOJQ) 

ax2.plot(df_GPCJ.iloc[:,2],df_GPCJ.iloc[:,13],linewidth=1.5,zorder=1, label = "REF.noOML",color=color_GKCJ) 
#obs
ax2.plot(df_GPCJ.iloc[:,2],df_obs.pmerobs[14:31], linewidth=1.5,zorder=1, label = "Obs",color='red')
...
figname4 = dir_im+nom_cyclone+'_'+date+'_Pmin_obs_glob01_GPGP.png'
figure4.savefig(figname4,dpi=170, format='png',bbox_inches=None, pad_inches=0.1)
...
ax1.plot(df_GP7E.iloc[:,2],df_GP7E.iloc[:,7],linewidth=1.5,zorder=1, label = "Xp.OML.buoys",color=color_GO4A) 
ax1.plot(df_GP84.iloc[:,2],df_GP84.iloc[:,7],linewidth=1.5,zorder=1, label = "Xp.OML.buoys.off",color=color_GP84) 
ax1.plot(df_GP84.iloc[:,2],df_GPGP.iloc[:,7],linewidth=1.5,zorder=1, label = "Xp.OML.buoys.R.10x4",color=color_GOJQ) 

ax1.plot(df_GPCJ.iloc[:,2],df_GPCJ.iloc[:,7],linewidth=1.5,zorder=1, label = "REF.noOML",color=color_GKCJ) 
#obs
ax1.plot(df_GPCJ.iloc[:,2],df_obs.ventobs[14:31], linewidth=1.5,zorder=1, label = "obs",color='red')
...
figname8 = dir_im+nom_cyclone+'_'+date+'_Vmax10_obs_GPGP.png'
