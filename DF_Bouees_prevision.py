...
raw_data =      {'date_valid': [''],
                 ...
                 'Nb_bouees_an':[''],
                 'diff_Nb':['']}
df_bouees_for = pd.DataFrame(raw_data, columns = ['date_valid','date_xp','expe','level','ech','biais_obs','eqm','Nb_bouees_tot','Nb_bouees_an','diff_Nb'])
...
for e in range(len(expe)):
    ...
    for t in range(nb):
        ...
        new_row = {'date_valid':date_valid,'date_xp':date_expe, 'expe':expe[e], 'var':name[0],'level':level[0], 'ech':cterm_for,'biais_obs': biais_obs_mean,'eqm': biais_obs_std,'Nb_bouees_tot':Nb_obs ,'Nb_bouees_an':Nb_obs_assim,'diff_Nb':Nb_diff}    
...
df_bouees_for.to_csv(pathres+str(zone)+'_P'+str(cterm_for)+'_bouees_flag.csv')  
...
# ax[1].legend(['Xp.final','GO4A'],fontsize=14, loc='upper right')
# plt.show()
