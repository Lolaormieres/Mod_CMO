...
dir_fig2='/cnrm/recyf/Data/users/ormieresl/plot_score_water_column_temp_v2/'
dir_fig3='/cnrm/recyf/Data/users/ormieresl/plot_score_water_column_temp_v3/'
...
#%% TEMP EXPE
raw_data = {'ech': ['']}
df_exp = pd.DataFrame(raw_data, columns = ['ech']) ## dataframe stat moyennes
...
for izone in range(len(zone)):
    ...
    for niv in name:
        ...
        ax.set_title('Xp.OML, '+str(zone_name[izone]),fontsize=26,y=1.06)
        ax.set_xlabel('Forecast range (UTC)',fontsize=22)
        ax.set_xticks(lst,
                labels=xlabels)
        axs.set_ylabel('Depth (m)',fontsize=22)
    
        
    ## PLOT PROPRE
        #cbar_ax = fig.add_axes([0.3, -0.17, 0.5, 0.08])  # [left,bottom,width,height]
        cbar_ax = fig.add_axes([1.05, 0.22, 0.02, 0.6]) # vertical
        ...
        clb.ax.set_title('(°C)',fontsize=20, y=1.10)
        ...
        # fig.suptitle('2-months averaged temperature in 1d model - '+str(expe), horizontalalignment = 'center',fontsize=30, y=1.02)
    
    print('******************************************************',Z)
    
    df_exp=pd.DataFrame(Z)
    df_exp.to_csv(dir_fig3+str(zone[izone])+'_CMO_september_octobre.csv')
    ...
    figname = dir_fig3+str(expe[0])+'_xptemp_column_ech_rainbow_HN_'+str(zone[izone])+'.png'
    fig.savefig(figname,dpi=150, format='png',bbox_inches='tight')
    figname = dir_fig3+str(expe[0])+'_xptemp_column_ech_rainbow_HN_'+str(zone[izone])+'.pdf'
    fig.savefig(figname,dpi=150, format='pdf',bbox_inches='tight')

#%%
raw_data = {'ech': ['']}
df_merc = pd.DataFrame(raw_data, columns = ['ech']) ## dataframe stat moyennes
...
for izone in range(len(zone)):
    ...
    for niv in name:
        ...
        ax.set_title('Mercator.instantaneous, '+str(zone_name[izone]),fontsize=26,y=1.06)
        ax.set_xlabel('Valid time (UTC)',fontsize=22)
        ax.set_xticks(lst,
                labels=xlabels)
        axs.set_ylabel('Depth (m)',fontsize=22)
    ...
    ## PLOT PROPRE
        cbar_ax = fig.add_axes([1.05, 0.22, 0.02, 0.6]) 
        CS1=ax.contourf(X, Y, Z,cmap='rainbow', levels=bar_tick[izone],linewidths=1)
        clb=fig.colorbar(CS1, spacing='uniform', cax=cbar_ax, orientation='vertical')
        cbar_ax.tick_params(labelsize=20)
        ax.tick_params(axis='y', labelsize=22)
        ax.tick_params(axis='x', labelsize=22)
        ax.tick_params(width=3, length=7)
        clb.ax.set_title('(°C)',fontsize=20, y=1.1)
       
        # plt.legend()
        fig.colorbar(CS1, cax=cbar_ax, orientation='vertical')
        # fig.suptitle('Evolution du biais de température par rapport à la Reference Mercator -'+str(expe), horizontalalignment = 'center',fontsize=15)
        # fig.suptitle('2-months Averaged temperature in Mercator - '+str(expe), horizontalalignment = 'center',fontsize=28, y=1.02)
   
   
    df_merc=pd.DataFrame(Z)
    df_merc.to_csv(dir_fig3+str(zone[izone])+'_MERC_september_octobre.csv')  
    
    print('min',min(temp))
    print('max',max(temp))
    print('averaged',np.mean(temp))

    figname = dir_fig3+str(expe[0])+'_xpmercator_column_ech_rainbow_HN_'+str(zone[izone])+'.png'
    fig.savefig(figname,dpi=150, format='png',bbox_inches='tight')
    figname = dir_fig3+str(expe[0])+'_xpmercator_column_ech_rainbow_HN_'+str(zone[izone])+'.pdf'
    fig.savefig(figname,dpi=150, format='pdf',bbox_inches='tight')