...
#Diff journaliere
nb=1
date_init=datetime(2022,9,16,0)
date=date_init
dt=timedelta(days=4)
...
#     try:
#         # r = get_file(expe, date, cblock, ckind, cfields, cmodel, cut, cterm)
#         r_for=get_file(xpref,date,cblock_for,ckind_for,cfields,cmodel,cut_for,cterm_for)
#         r_mer = get_file(xpref, date_valid, cblock_oc, ckind_oc,cfields_oc, cmodel_oc, cut_oc, cterm_oc)
#         if t == 0:
...
for t in range(nb):
    ...
    try:
        # r = get_file(expe, date, cblock, ckind, cfields, cmodel, cut, cterm)
        r_for=get_file(expe,date,cblock_for,ckind_for,cfields,cmodel,cut_for,cterm_for)
        r_for_ref=get_file(xpref,date,cblock_for,ckind_for,cfields,cmodel,cut_for,cterm_for)
        # r_mer = get_file(expe, date, cblock_oc, ckind_oc,cfields_oc, cmodel_oc, cut_oc, cterm)
        # r_ref = get_file(xpref, date, cblock, ckind, cfields, cmodel, cut, cterm)
        if t == 0:
