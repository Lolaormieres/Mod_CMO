...
raw_data = {'date_xp': [''],
            ...
            'cterm_mer':[''],
                ...
                'zone': [''],
                'compar': ['']}
df2 = pd.DataFrame(raw_data, columns = ['date_xp','ech','date_mer','cterm_mer','expe','var','level','biais','eqm','zone','compar'])

## FAire GKPH à la fin
# expe=['GL55','GKOR','GKPH','GM6E','GMOT']
# expe=['GL55']#,'GKPH','GKOR','GM6E','GMOT','GN84'] ## Eventuellement boucle sur exp
# expe=['GL55','GKPH','GKOR','GM6E','GMOT','GN84']
...
for e in range(len(expe)):
    ...
    date_valid=date_init-timedelta(hours=6)
    ...
    for t in range(nb):
        ...
        for i in range(len(name)):
            ...
            for h in range(len(cterm_for)):
                print('*********** ech', cterm_for[h])
                dt2=timedelta(hours=cterm_for[h])
                #date_valid= date_valid+dt2
                date_valid = date_reseau+dt2
                
                print('date_reseau',date_reseau)
                print('date_mer',date_valid)
                
                ### Cterm recuperer heure pour Mercator
                cterm=date_reseau.hour
                print('cterm reseau',cterm)
                cterm_mer=date_valid.hour
                print('cterm mer',cterm_mer)
                ...
                try:
                    ...
                    new_row = {'date_xp':date_2, 'ech':cterm_for[h],'date_mer':date_2_mer,'cterm_mer':cterm_mer, 'expe':expe[e], 'var':name[i],'level':level[i], 'biais':f_diff_mean,'eqm':f_diff_std,'zone':zone,'compar':compar}
...
df2.to_csv('DF_ech_4'+'/'+str(zone)+'_ech_septembre_octobre'+'_'+str(compar)+'.csv')
#%%
