...
Dir_im='/cnrm/recyf/Data/users/ormieresl/plot_sst_forecast_102h/'
...
expe=['GO48']
nb=17
date_init = datetime(2022, 9,1 , 18)
date = date_init
...
#%%
from datetime import datetime,timedelta
## PLOT CYCLE DIURNE - XP ATMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM
nb = 16  
# date_init = datetime(2022, 8,4, 0)
longg=360-13
longg=360-4
...
raw_data = {'date': [''],
            ...
            'lat': [''],
                ...
                'ssrd': [''],
                'ssrd_corr': ['']}
df_diurne_atm = pd.DataFrame(raw_data, columns = ['date','lon','lat','expe','var','level','ech','ssrd','ssrd_corr'])
# expe=['GK7C','GL55','GKOR','GKPH','GM6E','GMOT']
ssrd=[]
expe = ['GN84']
for e in range(len(expe)):
    print(expe[e]) 
    Pas='6h'
    cterm_for= [ '6', '12', '18', '24', '30', '36', '42', '48', '54', '60', '66', '72', '78', '84', '90', '96']
    for h in range(len(cterm_for)):
        print(cterm_for[h])
        # try:
        #r=get_file(expe,date,cblock,ckind,cfields,cmodel,cut,cterm)
        #r_ref=get_file(xpref,date,cblock,ckind,cfields,cmodel,cut,cterm)
        # r_for=get_file(expe[e],date,cblock_for,ckind_for,cfields,cmodel,cut_for,cterm_for[h],geometry,formats_for)
        #r_drown=get_file(expe_drown,date,cblock_oc,ckind_oc,cfields_oc,cmodel_oc,cut_oc,cterm)
        #r_mer=get_file(expe,date,cblock_oc,ckind_oc,cfields_oc,cmodel_oc,cut_oc,cterm)
        r_atm=get_file_atm(expe[0],date,cblock_atm,ckind_atm,cfields_atm,cmodel_atm,cut_atm,cterm_for[h],geometry_atm,formats_atm,fill_atm)
       
       
        for i in range(len(name_atm[i])):
            print(i,name_atm[i])
            x=r_atm.readfield(name_atm[i])
            lon_atm, lat_atm =x.geometry.get_lonlat_grid()

            mask=(lon_atm<longg)&(lon_atm>longg-1)&(lat_atm>lati)&(lat_atm<lati+1) 
            ...
            x_pt=x_total[0]
            print('x_pt',x_pt)
            lon_atm_pt=np.round(lon_atm[mask][0],2)
            lat_atm_pt=np.round(lat_atm[mask][0],2)
            print('lon atm',lon_atm_pt)
            print('lat atm',lat_atm_pt)

            # Champs non accumules 1pt
            ssrd_pt=x_total[0]
            ssrd=np.append(ssrd,ssrd_pt)
            if h==0:
                x_pt_corr=x_total[0]
                print('h=0',x_pt)
            else:
                x_pt_corr=x_pt-ssrd[h-1]
                print('xxxxxxx',x)

            date_2 = date.strftime("%d/%m/%y-%H:%M")
            new_row = {'date':date_2,'lon':lon_atm_pt, 'lat':lat_atm_pt, 'expe':expe[e], 'var':name_atm[i], 'ech':cterm_for[h],'ssrd': x_pt,'ssrd_corr': x_pt_corr}
            print('new row', new_row)    
            # Use the loc method to add the new row to the DataFrame
            df_diurne_atm.loc[len(df_diurne_atm)] = new_row
    # except:
            print('Pas de fichier')
    
# SUPRIMER 1er COL DF
df_diurne_atm.drop(index=df_diurne_atm.index[0], axis=0, inplace=True) ## DELETE FIRST LINE
#%%
...
for date_dt in toto:
    ...
    ind_flag = np.where((odb_key_val_dic['col_1']>long-3)&
...
## BIAIS
# ax.set_xticks([])
ax.set_ylabel('Bias (°C)',fontsize=20)
# xlabels = df_diurne_atm.ech[0:len(df_diurne_atm.ech):N]
# ax.set_xticks(xind,labels=xlabels,fontsize=15)

ax.set_xlabel('Forecast range (UTC)',fontsize=20)
ax.yaxis.set_label_coords(-0.15, .5) # place of labels
ax.xaxis.set_label_coords(0.5, -0.12)
ax.yaxis.set_tick_params(labelsize=16)
ax.xaxis.set_tick_params(labelsize=16)

ax.plot(SST_GN84.ech,GKCJ,color=color_GKCJ,label='oper.2024',linewidth=3)
# ax.plot(SST_GN84.ech,SST_GL55.SST,color=color_GL55,label='L.26',linewidth=3)
ax.plot(SST_GN84.ech,SST_GN84.SST,color=color_GN84,label='Xp.finale',linewidth=3)
ax.plot(SST_GN84.ech,SST_GO4A.SST,color=color_GO4A,label='Xp.finale.buyos',linewidth=3)
ax.plot(SST_GN84.ech,SST_GO4A_an.SST,color=color_GO4A,label='GO4A Analyse',linewidth=3,linestyle='--')
ax.plot(SST_GN84.ech,SST_GOJQ.SST,color=color_GOJQ,label='GOJQ.R10x4',linewidth=3)
...
plt.legend(fontsize=17, bbox_to_anchor=(1.05, 0.70))
...
figname = Dir_im+str(Date_debut)[0:10]+'_lonlat_'+str(lon_expe)+'_'+str(lat_expe)+'_SST_evol_forecast.png'
...
SST_GL55 = df_diurne.loc[((df_diurne['expe']=='GL55'))][['date','expe','var','level','ech', 'SST']] 
SST_GO4A = df_diurne.loc[((df_diurne['expe']=='GO4A'))][['date','expe','var','level','ech', 'SST']] 
SST_GNSR = df_diurne.loc[((df_diurne['expe']=='GNSR'))][['date','expe','var','level','ech', 'SST']] 
...
xlabels = df_diurne_atm.ech[0:len(df_diurne_atm.ech):N]
ax.set_xticks(xind,labels=xlabels,fontsize=15)
ax.set_xlabel('Term (hours)',fontsize=18)
ax.yaxis.set_tick_params(labelsize=18)

# ax2.set_ylabel('Secondary Y-axis', color= 'blue') 
# ax2.tick_params(axis='y', color='blue', labelcolor='blue')
ax2 = ax.twinx()
ax2.yaxis.set_tick_params(labelsize=18,labelcolor='coral')
ax2.set_ylabel('SW downward radiation',fontsize=16,color='coral')
# ax2.set_color('yellow')

# df_diurne_atm = df_diurne_atm.astype(int)

ax.plot(SST_GN84.ech,GKCJ,color=color_GKCJ,label='oper.2024',linewidth=3)
# ax.plot(SST_GN84.ech,SST_GL55.SST,color=color_GL55,label='L.26',linewidth=3)
ax.plot(SST_GN84.ech,SST_GN84.SST,color=color_GN84,label='Xp.finale',linewidth=3)
ax.plot(SST_GN84.ech,SST_GO4A.SST,color=color_GO4A,label='GO4A',linewidth=3)
ax.plot(SST_GN84.ech,SST_GO4A_an.SST,color=color_GO4A,label='GO4A Analyse',linewidth=3,linestyle='--')
# ax.plot(SST_GN84.ech,SST_GNSR.SST,color=color_GNSR,label='GNSR',linewidth=3)
ax.plot(SST_GN84.ech,df_diurne_oc.SST,color=color_GN3C,label='SST.Mercator',linewidth=3)
ax.plot(SST_GN84.ech,df_diurne_bouees.SST,color='lightskyblue',label='Obs.bouée', linewidth=3)

ax2.plot(df_diurne_atm.ech,df_diurne_atm.ssrd_corr, color='coral',linewidth=1,linestyle=':')
ax.legend(loc='best')
fig.suptitle('SST evolution over the forecast period - 102h - of the '+str(Date_debut)[0:10]+'(lon:'+str(lon_expe)+' lat:'+str(lat_expe)+')',fontsize=16,y=0.96)
plt.show()

figname = Dir_im+str(Date_debut)[0:10]+'_lonlat_'+str(lon_expe)+'_'+str(lat_expe)+'_SST_evol_forecast_radiation.png'
fig.savefig(figname,dpi=300, format='png',bbox_inches='tight')
...
# SST_GKCJ = df_diurne.loc[((df_diurne['expe']=='GKCJ') )][['date','expe','var','level','ech', 'SST']]
# GKCJ=SST_GKCJ.SST.iloc[0]
# GKCJ=np.linspace(GKCJ,GKCJ,17)

# SST_GL55 = df_diurne.loc[((df_diurne['expe']=='GL55'))][['date','expe','var','level','ech', 'SST']] 
# SST_GN84 = df_diurne.loc[((df_diurne['expe']=='GN84'))][['date','expe','var','level','ech', 'SST']] 
# SST_GKPH = df_diurne.loc[((df_diurne['expe']=='GKPH'))][['date','expe','var','level','ech', 'SST']] 
# SST_GO4A = df_diurne.loc[((df_diurne['expe']=='GO4A'))][['date','expe','var','level','ech', 'SST']] 
# SST_GNSR = df_diurne.loc[((df_diurne['expe']=='GNSR'))][['date','expe','var','level','ech', 'SST']] 
...
# xlabels = df_diurne_atm.ech[0:len(df_diurne_atm.ech):N]
# ax.set_xticks(xind,labels=xlabels,fontsize=13)
# ax.set_xlabel('Term (hours)',fontsize=16)
...
# # ax2.tick_params(axis='y', color='blue', labelcolor='blue')
# ax2 = ax.twinx()
# ax2.yaxis.set_tick_params(labelsize=14,labelcolor='green')
# ax2.set_ylabel('10u (m.s⁻1)',fontsize=16,color='green')
# # ax2.set_color('yellow')

# # df_diurne_atm = df_diurne_atm.astype(int)

# ax.plot(SST_GN84.ech,GKCJ,color=color_GKCJ,label='Double',linewidth=3)
# ax.plot(SST_GN84.ech,SST_GL55.SST,color=color_GL55,label='L.26',linewidth=3)
# ax.plot(SST_GN84.ech,SST_GO4A.SST,color=color_GO4A,label='GO4A',linewidth=3)
# ax.plot(SST_GN84.ech,SST_GNSR.SST,color=color_GNSR,label='GNSR',linewidth=3)
# ax.plot(SST_GN84.ech,SST_GN84.SST,color=color_GN84,label='Xp.finale',linewidth=3)
...
# ax2.plot(df_diurne_atm.ech,df_diurne_atm.ssrd, color='green',linewidth=2)
# ax.legend(loc='best')
# fig.suptitle('SST evolution over the forecast period - 102h - of the '+str(Date_debut)[0:10]+'(lon:'+str(long)+' lat:'+str(lati)+')',fontsize=16,y=0.96)
# plt.show()
...
# fig, ax = plt.subplots(nrows = 1,ncols = 1, sharex = False, figsize=(8, 5),dpi=300) 
...
# xx=lst
# xind = xx[0:len(xx):N] # Pas des dates plot
# xlabels = df_diurne.ech[0:len(df_diurne.ech):N]            
...
# # ax2.tick_params(axis='y', color='blue', labelcolor='blue')
# ax2 = ax.twinx()
# ax2.yaxis.set_tick_params(labelsize=14,labelcolor='coral')
# ax2.set_ylabel('SW downward radiation',fontsize=16,color='coral')
# # ax2.set_color('yellow')

# # df_diurne_atm = df_diurne_atm.astype(int)

# ax.plot(SST_GKPH.ech,GKCJ,color=color_GKCJ,label='Double',linewidth=3)
# # ax.plot(SST_GKPH.ech,SST_GKPH.SST,color=color_GKPH,label='R.50',linewidth=3)

# ax.plot(SST_GN84.ech,SST_GN84.SST,color=color_GN84,label='Xp.finale',linewidth=3)
# ax.plot(SST_GN84.ech,df_diurne_oc.SST,color=color_GN3C,label='SST.Mercator',linewidth=3)
# ax.plot(SST_GN84.ech,df_diurne_bouees.SST,color='lightskyblue',label='Obs.bouée', linewidth=3)

# ax2.plot(df_diurne_atm.ech,df_diurne_atm.ssrd_corr, color='coral',linewidth=2)
# ax.legend(loc='best')
# fig.suptitle('SST evolution over the forecast period - 102h - of the '+str(Date_debut)[0:10]+'(lon:'+str(long)+' lat:'+str(lati)+')',fontsize=16,y=0.96)
# plt.show()


# print('lon_bouees',np.round(lon_bouees[0],2))
# print('lat_bouees',np.round(lat_bouees[0],2))