...
def get_file(xpid, date, rep, typfic, fields, mod, chain, ech):
    ...
    fp = {'experiment': xpid, 'kind': typfic, 'block': rep, 'fields': fields,
          ...
          'local': 'tmpsst.fic', 'cutoff': chain, 'vapp': 'arpege',
          'vconf': '4dvarfr', 'model': mod, 'namespace': 'vortex.multi.fr', 'filling': 'surf'}
    fcref = toolbox.input(**fp)[0]
    fcref.get()
    r = epygram.formats.resource(filename='tmpsst.fic', openmode='r')
    os.remove('tmpsst.fic')
...
for zon in zones:
    ...
    print(zones[zon])
    df={}
    df[zon] = pd.DataFrame(raw_data, columns = ['date','expe','var','level','biais','eqm','ech','zone','compar','SST_xp', 'SST_mer'])
    print('dffffffffffff',df[zon])
...
### Boucle sur exp, date + niveaux
        ...
        for t in range(nb):
               ...
               for i in range(len(name)):
                    ...
                    SST_xp_tot=r.readfield(name_sst[i]) - 273.15 # SST arp Kelvin, TempOC °C
                    ...
                    mask = (lon >zones[zon][0]) & (lon < zones[zon][1]) & (lat > zones[zon][2]) & (lat < zones[zon][3])
                    print('massssssssk',mask)
                    f_diff_mean = f_diff.data[mask].mean()
                    f_diff_std = f_diff.data[mask].std()
                    SST_xp = SST_xp_tot.data[mask].mean()
                    SST_mer = SST_mer_tot.data[mask].mean()
                    ...
                    new_row = {'date':date_2, 'expe':expe[e], 'var':name[i],'level':level[i], 'biais':f_diff_mean,'eqm':f_diff_std,'zone':key,'compar':compar,'SST_xp':SST_xp,'SST_mer':SST_mer}

                    # Use the loc method to add the new row to the DataFrame
                    df[zon].loc[len(df[zon])] = new_row
                    ...
                    # r_mer.close()
                        ...
                        # # Define the new row to be added
                        # new_row = {'date':date_2, 'expe':expe[e], 'var':name_sst[i],'level':level[i], 'biais':f_diff_mean,'eqm':f_diff_std, 'ech':cterm,'zone':key,'compar':compar, 'SST_xp':SST_xp,'SST_mer':SST_mer}
                        # print('neeeeew row',new_row)
                        # #Use the loc method to add the new row to the DataFrame
...
for zon in zones.keys():
        print('zon',zon)
        print('df[zon]',df[zon])
        df[zon].to_csv(Dir+"/"+str(zon)+'_'+str(compar)+'.csv')
print('TERMINE 1')
...
for e in range(len(expe)):
    df2={}
    for zon in zones.keys():
        df2[zon] = pd.DataFrame(raw_data, columns = ['date','expe','var','level','biais','eqm','ech','zone','compar','SST_xp', 'SST_mer'])
    ...
    for t in range(nb):
            ...
            for i in range(len(name)):
               ...
               try:
                   ...
                   # r_mer = get_file(expe_mer[e], date_valid, cblock_oc, ckind_oc,cfields_oc, cmodel_oc, cut_oc, cterm) ## Mercator
                   r_mer = get_file(expe_mer[e], date_valid, cblock_oc, ckind_oc,cfields_oc, cmodel_oc, cut_oc, cterm)
               
                   SST_xp_tot=r_for.readfield(name_sst[i]) - 273.15 # SST arp Kelvin, TempOC °C
                   SST_mer_tot=r_mer.readfield(name_mer[i]) - 273.15  # Surf theta Kelvin
                   ...
                   for key in zones.keys():
                       ...
                       # Define the new row to be added
                       new_row = {'date':date_2, 'date_valid':date_valid_2, 'expe':expe[e], 'var':name_sst[i],'level':level[i], 'biais':f_diff_mean,'eqm':f_diff_std, 'ech':cterm,'zone':key,'compar':compar, 'SST_xp':SST_xp,'SST_mer':SST_mer}
                       # Use the loc method to add the new row to the DataFrame
                       df2[key].loc[len(df[key])] = new_row
                   r.close()
                   r_mer.close()
               except:
                   print('Pas de fichier')
...
for zon in zones.keys():
    print('zon',zon)
    print('df2[zon]',df[zon])
    df2[zon].to_csv(Dir+"/"+str(zon)+'_'+str(compar)+'.csv')
print('TERMINE 1')
